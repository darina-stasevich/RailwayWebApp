    ...
    ILogger<CarriagesController> logger) : ControllerBase
{
    ...
    public async Task<ActionResult<List<ShortCarriageInfoDto>>> GetCarriageSummaries([FromBody] CarriagesInfoRequest request)
    {
        ...
        try
        {
            ...
            var carriages = await carriageService.GetAllCarriagesInfo(
                request.ConcreteRouteId,
                request.StartSegmentNumber,
                request.EndSegmentNumber
            );
    ...
    public async Task<ActionResult<DetailedCarriageInfoDto>> GetCarriageDetails([FromBody] CarriageInfoRequest request)
    {
        ...
        try
        {
            logger.LogInformation("Getting carriages info for ConcreteRouteId: {ConcreteRouteId}, StartSegmentNumber: {StartSegmentNumber}, EndSegmentNumber: {EndSegmentNumber}, CarriageNumber: {CarriageNumber}",
                request.ConcreteRouteId, request.StartSegmentNumber, request.EndSegmentNumber, request.CarriageNumber);
            
            var carriage = await carriageService.GetCarriageInfo(
                request.ConcreteRouteId,
                request.StartSegmentNumber,
                request.EndSegmentNumber,
            ...
            logger.LogInformation("Successfully retrieved carriages for request: {CarriageRequest}", JsonSerializer.Serialize(request));
            return Ok(carriage);
        }
        catch (ArgumentException ex)
        {
